...
<br/>
<br/>
# Surge 1.9: Manuel d'utilisation
{:.no_toc}
<br/>
## Table des matières
{:.no_toc}
...
# Démarrage
Merci d'utiliser **Surge**\ !

Surge est un synthétiseur virtuel publié en open source par son créateur Claes Johanson
en septembre 2018, et maintenu par un groupe de bénévoles depuis.

Cette première section est destinée à vous donner un bref aperçu de certains concepts
spécifiques à ce synthétiseur et une introduction sur la façon de naviguer,
manipuler et utiliser Surge à son plein potentiel.

Pour des informations détaillées concernant le moteur de synthèse et d'autres spécifications
techniques avancées et options avancées de ce synthétiseur, une deuxième section
étant [Référence technique](#technical-reference) se trouve plus bas.

Enfin, pour plus de conseils et d'astuces, de tutoriels et pour télécharger du contenu supplémentaire,
vous pouvez également consulter le
[wiki de Surge](https://github.com/surge-synthesizer/surge-synthesizer.github.io/wiki).
...
*Audio Units, AU est une marque de commerce d'Apple Computer, Inc.  
VST est une marque de commerce de Steinberg Media Technologies GmbH*.

L'installateur de Surge est disponible à l'adresse
[https://surge-synthesizer.github.io](https://surge-synthesizer.github.io).


### Windows

Sous Windows, Surge est distribué sous forme de plug-in VST3 32 ou 64 bits.

Le nom de fichier pour le VST3 est `Surge.vst3`.

Spécéfications minimales requises :

  - Windows 7 ou plus récent
  - Un processeur raisonnablement rapide (Pentium 4/Athlon 64 ou supérieur)
  - Au moins 4GB de RAM
  - Application hôte compatible VST

De plus, pour utiliser la version 64 bits sous Windows, vous avez besoin des éléments suivants :

  - Un CPU supportant le jeu d'instructions x64 (AMD64/EM64T)
  - Une version **64-bit** de Windows
  - Une application capable d'héberger des plug-ins VST 64 bits.

La version VST3 du plug-in devrait être automatiquement installée dans l'emplacement par défaut de
plug-ins VST3 et devrait être trouvée par votre application hôte.
Cependant, la version Windows est également fournie avec un **mode portable** :

- Le mode portable vous permet de stocker les ressources de Surge dans le même répertoire que votre surge.vst3.
- Si Surge.vst3 est installé dans un dossier et que dans ce même dossier il y a un répertoire appelé `SurgeData`,
Surge l'utilisera pour ses données plutôt que `%PROGRAMDATA%\Surge`.
- Si dans le même dossier, il y a un répertoire appelé `SurgeUserData`, Surge l'utilisera pour les données
utilisateur plutôt que `%DOCUMENTS%\Surge`.
- Soit aucun, soit un, soit les deux dossiers peuvent être là.
Surge reviendra aux valeurs par défaut s'ils ne sont pas présents.
Vous pouvez toujours voir vos chemins de données dans l'écran [about](#about-surge).


<br/>
### macOS

Sur Mac, Surge est livré en tant qu'instrument plug-in 64 bits pour les plug-ins Audio
Unit (AU) et plug-in VST (VST3).

Spécéfications minimales requises :

  - Mac OS X 10.11 ou plus récent
  - Un processeur Intel 64 bits
  - Au moins 4 Go de RAM
  - Une application hôte 64 bits compatible AU ou VST

Pour installer Surge, exécutez le programme d'installation. Vous aurez la possibilité d'installer
automatiquement les composants AU `Surge.component` et le VST3 `Surge.vst3` à leurs emplacements corrects.
Les présélections par défaut et les wavetables seront également installées automatiquement.

L'exécution du programme d'installation packagé installera Surge pour tous les utilisateurs de votre
Mac.

<br/>
### Linux

Sur Linux, Surge est livré en tant que VST3 64 bits.

La configuration requise peut être difficile à déterminer, car il existe de nombreuses
distributions et autres facteurs. Cependant, il est bon de connaître les informations suivantes:

 - Le paquet d'installation sur le site Web de **Surge** est sous la forme d'un paquet Debian.
 - Le paquet de distribution est construit sur Ubuntu 16.04
 - Les paquets requis sont listés dans la source et dans le fichier deb.

**Note:** Certaines actions dans Surge se font en faisant Alt + Drag ou molette de défilement.
Sur certaines distributions Linux, ces raccourcis et gestes peuvent causer des conflits entre Surge
et l'environnement de bureau. Nous avons décidé que nous ne pouvions pas abandonner alt-drag pour
toutes les plateformes parce qu'un gestionnaire de fenêtres dans une distribution en particulier
l'utilisait par défaut. Par conséquent, il est souvent possible de désactiver ce geste global
dans les environnements de bureau, et ce serait le moyen le plus simple de résoudre ce potentiel problème.
...
Si vous souhaitez complier Surge à partir de la source, consultez les instructions sur
[notre dépôt Github] (https://github.com/surge-synthesizer/surge).
...
La bibliothèque de presets et les wavetables se trouvent à l'emplacement `C:ProgramData\Surge`.
Les presets de l'utilisateur se trouvent dans `C:\Users\votre nom d'utilisateur\Mes documents\Surge`.

### macOS

La bibliothèque de presets et les wavetables se trouvent à l'emplacement `/Library/Application Support/Surge`.
Les presets utilisateur se trouvent dans `~/Documents/Surge`.

### Linux

Le plugin lui-même, la bibliothèque de presets et les wavetables sont dans `/usr/share/Surge`
avec une installation standard. Les presets de l'utilisateur sont dans `~/Documents/Surge`.

<br/>

Note : Ces emplacements peuvent être modifiés dans le menu de Surge (voir [Data and Patches](#data-and-patches)).
...
L'interface utilisateur de Surge est divisée en quatre sections principales :
  - Patch/Global
  - Scene controls
  - Modulation/Routing
  - FX

En gardant cette structure en tête, il sera plus facile de comprendre la disposition des différents modules.

[Illustration 1 : Les quatre sections de l'interface utilisateur de Surge](./images/Pictures/sections.png)

*Les quatre sections de l'interface utilisateur en lesquelles Surge est divisé.*

<br/>

## Le concept de "Scène"

Chaque patch dans Surge contient deux scènes (A & B) et une section d'effet.
Les deux scènes et tous les réglages d'effet sont stockés dans chaque patch. Une scène est similaire à un
patch de synthétiseur traditionnel car elle stocke toutes les informations utilisées pour synthétiser une voix.
Comme il y a deux scènes dans chaque patch, il est possible d'avoir des sons superposés ou divisés stockés dans un seul patch.
(voir [Scene Select and Scene Mode](#scene-select-and-scene-mode)).

![Illustration 2 : Les deux scènes et tous les réglages d'effets sont stockés dans chaque patch](./images/Pictures/illu2.png).

<br/>

## Sorties audio

Lorsqu'elle est chargée dans un DAW, chaque instance de Surge dispose de **3 sorties audio** :
- Stereo Out
- Scene A Out
- Scene B Out

Dans certains hôtes, comme FL Studio par exemple, Surge désactivera les sorties de scènes individuelles en raison d'un conflit avec l'architecture de routage audio du DAW. Pour activer ces sorties supplémentaires dans de tels cas,
vous trouverez [cette option](#workflow) dans le menu Surge.

<br/>

## Curseurs et commandes

Le contrôle le plus courant de l'interface utilisateur dans Surge est le curseur. Bien qu'ils se retrouvent sous forme
horizontale et verticale, leur fonctionnalité est autrement identique.

Les curseurs sont toujours glissés, il n'y a pas de saut si vous cliquez en arrière du curseur au lieu de sur sa tête, 
il entre en mode de déplacement dans tous les cas.

Interactions avec les curseurs :  
  - **Déplacement par clic gauche** - Faire glisser le curseur
  - **Maj + clic gauche sur le curseur** - Faire glisser le curseur (finement)
  - **Ctrl/Cmd + Clic gauche sur le curseur** - Faire glisser le curseur (accrochage à certaines valeurs)
  - **Alt + Clic gauche sur le curseur** - Faire glisser le curseur en mode élastique (retour à la position initiale lors du relâchement).
  - **Molette de la sourie** - Déplacer le curseur
  - **Maj + molette de la sourie** - Déplacer le curseur (finement)
  - **Double-clic gauche** - Remettre le paramètre à sa valeur par défaut. 
  - **Clic droit** - Menu contextuel

Outre les curseurs, certains des paramètres de Surge sont également affichés sous forme de champs de chiffres et de valeurs, 
de boutons et de rangées de boutons.

<br/>

### Menu contextuel des paramètres

Le menu contextuel de tout paramètre peut être affiché par l'entremise un clic droit. Ce menu dispose de nombreuses 
fonctions utiles :

![Illustration 5 : Menu contextuel du curseur](./images/Pictures/slider_context_menu.png)

#### Nom et aide contextuelle
En cliquant sur cette première option, le manuel de l'utilisateur s'ouvre sur la section correspondante expliquant le
paramètre en question.

#### Modifier la valeur
Cette option vous permet de directement saisir la valeur souhaitée d'un paramètre. Lorsque la fenêtre popup de la valeur apparaît, 
son texte est déjà surligné, et vous pourrez commencer à taper la valeur immédiatement. 
Lorsque vous avez terminé, il vous suffit d'appuyer sur la touche Entrée pour confirmer la modification.
Pour annuler et fermer cette fenêtre, il suffit d'appuyer sur la touche Echap ou de déplacer un autre paramètre.

![](./images/Pictures/typein_window.png)

Pour les paramètres discrets (Voix d'unisson, ou une rangée de boutons par exemple), au lieu d'un champ de saisie, toutes les 
valeurs possibles seront affichées directement dans le menu pour pouvoir y accéder directement.

![](./images/Pictures/discrete_values.png)

Si un contrôle est modulé, il y aura également des options d'édition pour la quantité de modulation appliquée pour chaque source de modulation.
La valeur saisie correspond à la position du curseur de modulation (curseur bleu) pour cette source de modulation (voir [Routing](#routing) 
pour plus d'informations).

![](./images/Pictures/typein_modulation.png)

Notez que dans les deux cas, il n'est pas nécessaire d'inclure l'unité du paramètre.

#### Extend Range
Certains paramètres peuvent avoir leur plage étendue. L'option **Extend range** apparaîtra dans le menu contextuel
si c'est le cas. **Pitch**, par exemple, est l'un de ces paramètres.

#### Tempo Sync
Certains paramètres peuvent être synchronisés avec le tempo de du programme hôte. L'option **Tempo sync** apparaîtra dans le menu contextuel si c'est le cas.

Une fois synchronisés sur le tempo, lorsque vous utilisez le skin Surge Classic, les curseurs afficheront un symbole "TS" sur leurs poignées pour indiquer cet état, 
comme ci-dessous :

![](./images/Pictures/ts_slider.png)

Cette indication peut varier en fonction du skin utilisé.

#### Activate / Deactivate
Certains paramètres peuvent être activés ou désactivés. Si un curseur apparaît transparent ou n'a pas sa poignée,
dans certains cas, cela peut être parce que le paramètre est désactivé. Pour le faire basculer son état d'activation, il suffit d'utiliser cette option.

#### Assign parameter to...
Cette option permet d'assigner le paramètre cliqué avec le bouton droit de la souris à n'importe quel CC MIDI.

#### MIDI Learn Parameter...
C'est ici que vous assignez un contrôleur MIDI au contrôle souhaité. Pour interrompre l'apprentissage MIDI sur ce paramètre,
cliquez simplement à nouveau avec le bouton droit de la souris et l'option deviendra **Abort Parameter MIDI Learn**.

#### Clear learned MIDI (...)
Cette option sera disponible si le paramètre sélectionné a déjà été appris par MIDI. Elle vous permet d'effacer ce lien
(le numéro CC MIDI du lien existant sera affiché entre parenthèses).

#### Add modulation from...
Cette entrée de menu permet de moduler directement le paramètre cliqué avec le bouton droit à partir de n'importe quelle source de modulation dans Surge.
Une fois qu'une source est sélectionnée, une fenêtre pop-up apparaît et vous permet d'entrer la quantité de modulation désirée à partir de cette source.

#### Clear Modulation
Ce menu comprend également une option facilement accessible pour effacer un ou tous les liens de modulation vers un curseur en cours de modulation 
(ceux qui ont une teinte bleue) (voir [Routing](#routing)).

#### Options VST3
Enfin, la version VST3 de Surge prend en charge les éléments de menu contextuel VST3. Selon l'hôte utilisé,
il peut y avoir plus ou moins d'options concernant l'automatisation, le MIDI, ou les valeurs des paramètres.
...
Les boutons **Scene Select** **[A|B]** déterminent laquelle est sélectionnée et affichée.
...
En effet, le fait qu'une scène génère une voix lorsqu'une touche est pressée est déterminé par le paramètre **Scene Mode** :
  ...
  - **Key Split** - Les notes situées en dessous de la **note de split** seront jouées par la scène A,
    les notes situées sur et au-dessus de la **note de split** seront jouées par la scène B.
  - **Channel Split** - Les notes des canaux MIDI situés en dessous du **canal MIDI split** seront jouées par la scène A,
    les notes des canaux MIDI au-dessus et incluant le **canal MIDI split** seront jouées par la scène B.
  - **Dual** - Les deux scènes joueront toutes les notes.

En mode **Key Split** et **Dual**, si la fonction MPE est désactivée, le système prend également en charge le routage des 
canaux MIDI où le canal 2 joue uniquement la scène 1 et le canal 3 joue uniquement la scène 3. 
Le canal MIDI 1 et tous les autres canaux supérieurs à 3 jouent le mode Split/Dual.

**Poly** indique le nombre de voix en cours de processus et vous permet de fixer une limite supérieure au nombre de voix autorisées 
à jouer en même temps en faisant en même temps en faisant glisser horizontalement la valeur. Le limiteur de voix éliminera doucement 
les voix en excès pour éviter les artefacts audibles. Il n'est donc pas rare que le nombre de voix dépasse la limite.
...
### Naviguer dans les presets
Trouver des sons dans Surge est facile : il suffit d'appuyer sur les boutons fléchées sur l'interface jusqu'à ce que vous trouviez 
quelque chose que vous aimez. Si vous faites un clic gauche sur le champ d'information du patch (n'importe où dans la zone blanche), 
un menu listera tous les patches disponibles, classés en catégories. Un clic droit fait apparaître un menu contenant uniquement 
les patches de la catégorie actuelle.

Ces catégories sont également regroupées en trois sections en fonction de leur créateur :

  - Factory Patches - Patches créés par les auteurs de Surge.

  - 3<sup>rd</sup> party patches - Patches créés par les utilisateurs et tierces parties.
...
C'est là que vous nommez votre nouveau patch et que vous choisissez la catégorie à laquelle il appartiendra.
Vous pouvez également créer une nouvelle catégorie manuellement. Les patchs que vous stockez se retrouveront dans 
la section utilisateur au bas de la page du menu des patchs. La boîte de dialogue de stockage fournit également 
des champs de texte pour le nom du créateur du patch et les commentaires.
...
Par exemple, la première fois que vous appuyez sur le bouton **Tune** si aucun accordage personnalisé n'est chargé, le même menu s'ouvrira, 
comme si vous aviez fait un clic droit sur le bouton **Tune**. Cependant, une fois qu'un accordage personnalisé est chargé, un clic gauche 
sur ce bouton permet d'activer ou de désactiver l'accordage chargé.

Les fichiers **.scl** et **.kbm** peuvent également être glissés et déposés n'importe où sur l'interface pour importer un accordage personnalisé.

Voir [Accordage personnalisé](#microtonal-tuning) dans la section Référence technique pour plus d'informations.

<br/>

## FX-Bypass, caractère et volume global

![](./images/Pictures/fx_bypass.png)

**FX Bypass** vous permet d'entendre rapidement le son d'un patch sans les d'effets. (voir [Section FX](#fx-section))

  - **Off** - Le Bypass est désactivé, tous les effets sont actifs.
  - **Send** - Les effets send sont contournés.
  - **Send + Global** - Les effets send et globaux sont contournés.
  - **Tous** - Tous les effets sont contournés.



**Character** contrôle la quantité de contenu haute-fréquence présent dans la plupart des algorithmes d'oscillateur de Surge. 
Les choix disponibles sont Warm, Neutral et Bright.

**Global Volume** contrôle le dernier étage de gain avant la sortie. Le VU-mètre situé au-dessus indique le 
niveau de sortie et devient rouge s'il dépasse 0 dBFS. Vous pouvez choisir d'écrêter la sortie globale soit à
**+18 dBFS** (par défaut) ou **0 dBFS**.

L'état de ces deux réglages n'est **pas** stocké avec les patches. Ils
sont cependant stockés par l'application hôte dans vos projets.
...
# Section des contrôles de la scène

L'interface utilisateur de la section des scènes peut elle-même être divisée en deux parties :

  - La génération du son
  - Le sculptage du son

Le son est généré et mixé dans la section de génération du son. Ensuite, 
il passe par la section de sculptage du son.
...
C'est ici que le son est né. Les oscillateurs génèrent des formes d'onde
en fonction des notes jouées. Ils sont ensuite regroupés dans le mixer.
...
**1/2/3-boutons** - Choisit l'oscillateur actif pour l'édition. Vous pouvez faire un clic droit sur l'un d'entre eux et un menu contextuel avec le nom, les options **Copie** et 
**Copie (avec modulation)** apparaîtra.

**Display** - Affiche la forme d'onde active. Lorsque l'oscillateur **Wavetable** ou **Window** est utilisé, il agit également comme sélecteur de wavetable en cliquant sur la barre orange ou sur les flèches pour les faire défiler.

**Type** - Type d'oscillateur. Les options disponibles sont :
- Classic
- Modern
- Wavetable
- Window
...
Voir [Oscillator algorithms](#oscillator-algorithms) dans la section Référence technique pour plus d'informations.

**Pitch & Octave** - Contrôle le pitch de l'oscillateur en cours d'étition.
Son menu contextuel peut être utilisé pour étendre sa plage, ou pour passer en mode **Absolu**, ce qui fait que le pitch se 
déplace en fréquence absolue à la place d'être par fréquence relative à la note qui est jouée.

**Keytrack** - Lorsque cette option est désactivée, l'oscillateur jouera la même tonalité
peu importe la touche jouée. Ce bouton peut être cliqué avec le bouton droit pour basculer son état sur tous les oscillateurs de la scène.

**Retrigger** - S'il est actif, l'oscillateur et toutes ses voix d'unisson démarreront toujours immédiatement à la même position de phase.
Ceci est utile pour les sons rapides où vous voulez que l'attaque soit exactement la même à chaque note. Ce bouton peut être cliqué avec 
le bouton droit pour définir son état sur tous les oscillateurs de la scène.

**Autres** - Le reste des curseurs de l'éditeur d'oscillateur dépendent du type d'oscillateur.
Voir [Oscillator algorithms](#oscillator-algorithms) dans la section Référence technique pour plus d'informations.
